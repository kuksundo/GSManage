...
uses
  ...
  Vcl.StdCtrls, JvExControls, JvLabel, Vcl.ExtCtrls,
...
uses CommonData2, UnitHGSVDRData, UnitFolderUtil2,
  {$IFDEF GAMANAGER} UnitGAVarJsonUtil2,
  {$ELSE} UnitVariantJsonUtil,
  {$ENDIF}
  FrmGSFileList2, UnitJHPFileData;
...
//        VDRListGrid.CellByName['Attachments', VDRListGrid.SelectedRow].AsInteger := LFileCount;
...
begin
  ...
  AVDRSearchParamRec.fHullNo := System.SysUtils.Trim(HullNoEdit.Text);
  AVDRSearchParamRec.fShipName := System.SysUtils.Trim(ShipNameEdit.Text);
  AVDRSearchParamRec.fIMONo := System.SysUtils.Trim(ImoNoEdit.Text);
  AVDRSearchParamRec.fVDRSerialNo := System.SysUtils.Trim(VDRSerialNoEdit.Text);
end;
