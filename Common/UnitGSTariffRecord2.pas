...
uses
  ...
  mormot.rest.sqlite3,
  Generics.Collections, UnitGSTriffData, CommonData2;
...
function GetGSServiceRate(const ACompanyCode: RawUTF8; const AYear: integer; const AGSWorkType: TGSWorkType): integer;
...
//  LStr := ExtractFilePath(Application.ExeName) + 'GSTariff.sqlite';
  ...
  g_GSTariffDB:= TRestClientDB.Create(TariffModel, CreateGSTariffModel,
    LStr, TSQLRestServerDB);
  TRestClientDB(g_GSTariffDB).Server.CreateMissingTables;
end;
...
function GetGSTariffFromCompanyCode(const ACompanyCode: RawUTF8): TSQLGSTariff;
begin
  Result := TSQLGSTariff.CreateAndFillPrepare(g_GSTariffDB.Orm,
...
var
  LSQLGSTariff: TSQLGSTariff;
  LGSTariffSearchRec: TGSTariffSearchRec;
begin
  LGSTariffSearchRec := Default(TGSTariffSearchRec);
  LGSTariffSearchRec.fCompanyCode := ACompanyCode;
  LGSTariffSearchRec.fYear := AYear;
  LGSTariffSearchRec.fGSWorkType := AGSWorkType;

  LSQLGSTariff := GetGSTariffFromSearchRec(LGSTariffSearchRec);
  try
    if LSQLGSTariff.IsUpdate then
      Result := LSQLGSTariff.GSServiceRate
    else
      Result := 0;
  finally
    LSQLGSTariff.Free;
...
begin
  ...
  try
    ...
    Result := TSQLGSTariff.CreateAndFillPrepare(g_GSTariffDB.Orm, Lwhere, ConstArray);
