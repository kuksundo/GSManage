...
  mormot.core.os, mormot.orm.core, mormot.orm.base, mormot.rest.client,
  mormot.core.datetime, mormot.rest.sqlite3,
  CommonData2,
  UnitTodoCollect2, FSMClass_Dic, FSMState, UnitJHPFileRecord,
  UnitVesselData2, UnitEngineMasterData, UnitGSTriffData, UnitGAServiceData,
...
//  TSQLGSFiles = class;
  ...
  private
    ...
    fNation, fCity, FURL, FTaxID, FStatus, fBusinessCondition, fBusinessSector,
    fERPConfirm: RawUTF8;
    fCompanyTypes: TCompanyTypes;
    fCompanyTypes2: TCompanyTypes2;
  ...
  published
    ...
    property CompanyName2: RawUTF8 read fCompanyName2 write fCompanyName2;//국내업체일 경우 한글명, 해외업체일 경우 약어(인증서 인쇄시 글자수 제한 필요함)
...
    property TaxID: RawUTF8 read FTaxID write FTaxID;
    property Status: RawUTF8 read FStatus write FStatus;
    property BusinessCondition: RawUTF8 read fBusinessCondition write fBusinessCondition; //업태
    property BusinessSector: RawUTF8 read fBusinessSector write fBusinessSector; //업종
    property ERPConfirm: RawUTF8 read fERPConfirm write fERPConfirm;
...
    property CompanyTypes2: TCompanyTypes2 read fCompanyTypes2 write fCompanyTypes2;
...
    property RegisteredDate: TTimeLog read fRegisteredDate write fRegisteredDate;
  ...
  private
    fAgentInfo,
    fDefaultHullNo,
    fDefaultProjNo: RawUTF8;
  published
    property AgentInfo: RawUTF8 read fAgentInfo write fAgentInfo;
    property DefaultHullNo: RawUTF8 read fDefaultHullNo write fDefaultHullNo;
    property DefaultProjNo: RawUTF8 read fDefaultProjNo write fDefaultProjNo;
...
function CreateCompanyMasterModel: TSQLModel;
...
function GetMasterCustomerFromCompanyCodeNName2Var(ACompanyCode, ACompanyName: string;
  ACompanyType: TCompanyTypes = []; AElecProductDetailTypes: TElecProductDetailTypes = []): Variant;
...
procedure AddOrUpdateMasterCustomerFromVariant(ADoc: variant);
...
procedure LoadGSFileFromVariant(LGSFile:TOrmJHPFile; ADoc: variant);
...
uses SysUtils, Forms, TodoList, VarRecUtils, UnitGAVarJsonUtil2,
  UnitFolderUtil2, UnitRttiUtil2, UnitJHPFileData;
...
//  LStr := ChangeFileExt(Application.ExeName,'.db3');
  ...
  g_ProjectDB:= TRestClientDB.Create(ProjectModel, CreateProjectModel,
    LStr, TRestServerDB);
  TRestClientDB(g_ProjectDB).Server.CreateMissingTables;

  LStr2 := LStr.Replace('.db3', '_Detail.db3');
  ProjectDetailModel := CreateProjectDetailModel;
  g_ProjectDetailDB:= TRestClientDB.Create(ProjectDetailModel, CreateProjectDetailModel,
    LStr2, TRestServerDB);
  TRestClientDB(g_ProjectDetailDB).Server.CreateMissingTables;

  LStr2 := LStr.Replace('.db3', '_SubConInvoice.db3');
  SubConInvoiceModel := CreateSubConInvoiceModel;
  g_SubConInvoiceDB:= TRestClientDB.Create(SubConInvoiceModel, CreateSubConInvoiceModel,
    LStr2, TRestServerDB);
  TRestClientDB(g_SubConInvoiceDB).Server.CreateMissingTables;

//  LStr2 := LStr.Replace('.db3', '_Master.db3');
//  MasterModel := CreateMasterModel;
//  g_MasterDB:= TRestClientDB.Create(MasterModel, CreateMasterModel,
//    LStr2, TRestServerDB);
//  TRestClientDB(g_MasterDB).Server.CreateMissingTables;

  InitJHPFileClient(Application.ExeName);
  InitCompanyMasterClient(AExeName);
end;
...
begin
  ...
  LStr := GetSubFolderPath(LStr, 'db');
  LStr := EnsureDirectoryExists(LStr);
  LStr := LStr + ADBName; //'CompanyMaster.sqlite'

  MasterModel := CreateCompanyMasterModel;
...
begin
  ...
  begin
    Result:= TSQLGSTask.Create(g_ProjectDB.Orm, ATaskID);
...
function GetTaskFromHullNoNPONo(AHullNo, APONo: string): TSQLGSTask;
begin
  Result := TSQLGSTask.CreateAndFillPrepare(g_ProjectDB.Orm,
...
function GetCustomerFromTask(ATask: TSQLGSTask): TSQLCustomer;
begin
  Result := TSQLCustomer.CreateAndFillPrepare(g_ProjectDetailDB.Orm, 'TaskID = ?', [ATask.ID]);
...
function GetCustomerFromTaskID(const ATaskID: TID): TSQLCustomer;
begin
  Result := TSQLCustomer.CreateAndFillPrepare(g_ProjectDetailDB.Orm, 'TaskID = ?', [ATaskID]);
...
function GetSubConFromTask(ATask: TSQLGSTask): TSQLSubCon;
begin
  Result := TSQLSubCon.CreateAndFillPrepare(g_ProjectDetailDB.Orm, 'TaskID = ?', [ATask.ID]);
...
function GetSubConFromTaskID(ATaskID: TID): TSQLSubCon;
begin
  Result := TSQLSubCon.CreateAndFillPrepare(g_ProjectDetailDB.Orm, 'TaskID = ?', [ATaskID]);
...
function GetSubConFromTaskIDNSubConID(ATaskID, ASubConID: TID): TSQLSubCon;
begin
  Result := TSQLSubCon.CreateAndFillPrepare(g_ProjectDetailDB.Orm, 'TaskID = ? and SubConID = ?', [ATaskID, ASubConID]);
...
function GetSubConFromSubConID(ASubConID: TID): TSQLSubCon;
begin
  Result := TSQLSubCon.CreateAndFillPrepare(g_ProjectDetailDB.Orm, 'SubConID = ?', [ASubConID]);
...
function GetSubConFromUniqueSubConID(AUniqueSubConID: RawUTF8): TSQLSubCon;
begin
  Result := TSQLSubCon.CreateAndFillPrepare(g_ProjectDetailDB.Orm, 'UniqueSubConID = ?', [AUniqueSubConID]);
...
function GetSubConFromCompanyCode(ACode: string): TSQLSubCon;
begin
  Result := TSQLSubCon.CreateAndFillPrepare(g_ProjectDetailDB.Orm, 'CompanyCode = ?', [ACode]);
...
function GetSubConInvoiceItemFromSubConID(ASubConID: TID): TSQLSubConInvoiceItem;
begin
  Result := TSQLSubConInvoiceItem.CreateAndFillPrepare(g_SubConInvoiceDB.Orm, 'SubConID = ?', [ASubConID]);
...
function GetMaterial4ProjFromTask(ATask: TSQLGSTask): TSQLMaterial4Project;
begin
  Result := TSQLMaterial4Project.CreateAndFillPrepare(g_ProjectDetailDB.Orm, 'TaskID = ?', [ATask.ID]);
...
function GetMaterial4ProjFromTaskID(const ATaskID: TID): TSQLMaterial4Project;
begin
  Result := TSQLMaterial4Project.CreateAndFillPrepare(g_ProjectDetailDB.Orm, 'TaskID = ?', [ATaskID]);
...
//    end;
    ...
    Result := TSQLMasterCustomer.CreateAndFillPrepare(g_MasterDB.Orm, Lwhere, ConstArray);
...
begin
  LSQLMasterCustomer := GetMasterCustomerFromCompanyCodeNName(ACompanyCode,
    ACompanyName,ACompanyType,AElecProductDetailTypes);
  try
    if LSQLMasterCustomer.IsUpdate then
    begin
      TDocVariant.New(Result, [dvoReturnNullForUnknownProperty]);
      LoadRecordPropertyToVariant(LSQLMasterCustomer, Result);
    end
    else
     Result := null;
  finally
...
function GetToDoItemFromTask(ATask: TSQLGSTask): TSQLToDoItem;
begin
  Result := TSQLToDoItem.CreateAndFillPrepare(g_ProjectDB.Orm, 'TaskID = ?', [ATask.ID]);
...
begin
  ...
  begin
    ATask.EmailMsg.ManyDelete(g_ProjectDB.Orm, ATask.ID, LIds[i]);
...
var
  LSQLMasterCustomer: TSQLMasterCustomer;
begin
  LSQLMasterCustomer := GetMasterCustomerFromCompanyCodeNName(ADoc.CompanyCode, ADoc.CompanyName);
...
//  AToDoCollect.Clear;
  ...
  LSQLToDoItem := TSQLToDoItem.CreateAndFillPrepare(g_ProjectDB.Orm, 'TaskID = ?', [ATask.ID]);
  try
    while LSQLToDoItem.FillOne do
    begin
...
//    LGSFile.FillRewind;
  ...
  begin
    ...
    LJson := LVar;
    RecordLoadJson(LGSFileRec, LJson, TypeInfo(TOrmJHPFile));
...
begin
  ...
  LoadRecordPropertyFromVariant(AMasterCustomer, ADoc);
...
//  AMasterCustomer.ManagerName := ADoc.ManagerName;
  ...
  Li := -1;
  LStr := TDocVariantData(ADoc).GetValueOrEmpty('CompanyTypes');
  ...
  begin
    LStr := TDocVariantData(ADoc).GetValueOrEmpty('CompanyTypes2');

    if LStr <> '' then
    begin
      LCompanyTypes2 := GetCompanyTypes2FromCommaString(LStr);
    end;
  end;

  LStr :=  TDocVariantData(ADoc).GetValueOrEmpty('BusinessAreas');

  if LStr <> '' then
  begin
    Li2 := StrToIntDef(LStr, 0);
  end;

  if AMasterCustomer.IsUpdate then
  begin
    if Li <> -1 then
      AMasterCustomer.CompanyTypes := AMasterCustomer.CompanyTypes + IntToTCompanyType_Set(Li)
    else
      AMasterCustomer.CompanyTypes2 := AMasterCustomer.CompanyTypes2 + LCompanyTypes2;
    AMasterCustomer.BusinessAreas := AMasterCustomer.BusinessAreas + IntToTBusinessArea_Set(Li2);
  end
  else
  begin
    if Li <> -1 then
      AMasterCustomer.CompanyTypes := IntToTCompanyType_Set(Li)
    else
      AMasterCustomer.CompanyTypes2 := LCompanyTypes2;

    AMasterCustomer.BusinessAreas := IntToTBusinessArea_Set(Li2);
  end;
...
begin
  ...
  g_InvoiceProjectDB:= TRestClientDB.Create(InvoiceTaskModel, CreateInvoiceTaskModel,
    LStr, TRestServerDB);
  TRestClientDB(g_InvoiceProjectDB).Server.CreateMissingTables;

  LStr := LStr.Replace('.db3', '_Items.db3');
  InvoiceItemModel := CreateInvoiceItemModel;
  g_InvoiceItemDB:= TRestClientDB.Create(InvoiceItemModel, CreateInvoiceItemModel,
    LStr, TRestServerDB);
  TRestClientDB(g_InvoiceItemDB).Server.CreateMissingTables;

  LStr := LStr.Replace('.db3', '_Files.db3');
  InvoiceFileModel := CreateInvoiceFilesModel;
  g_InvoiceFileDB:= TRestClientDB.Create(InvoiceFileModel, CreateInvoiceFilesModel,
    LStr, TRestServerDB);
  TRestClientDB(g_InvoiceFileDB).Server.CreateMissingTables;
end;
...
begin
  ...
  begin
    Result:= TSQLInvoiceTask.Create(g_InvoiceProjectDB.Orm, ATaskID);
    if Result.ID = ATaskID then
    begin
      Result.IsUpdate := True;
    end
...
function GetFilesFromInvoiceTask(ATask: TSQLInvoiceTask): TSQLInvoiceFile;
begin
  Result := TSQLInvoiceFile.CreateAndFillPrepare(g_InvoiceFileDB.Orm, 'TaskID = ?', [ATask.ID]);
end;

function GetFilesFromInvoiceItem(AItem: TSQLInvoiceItem): TSQLInvoiceFile;
begin
  Result := TSQLInvoiceFile.CreateAndFillPrepare(g_InvoiceFileDB.Orm, 'TaskID = ? and ItemID = ?', [AItem.TaskID, AItem.ItemID]);
end;

function GetFilesFromInvoiceIDList(AIDList: TIDList4Invoice): TSQLInvoiceFile;
begin
  Result := TSQLInvoiceFile.CreateAndFillPrepare(g_InvoiceFileDB.Orm, 'TaskID = ? and ItemID = ?', [AIDList.TaskID, AIDList.ItemId]);
end;

function GetItemsFromInvoiceTask(ATask: TSQLInvoiceTask): TSQLInvoiceItem;
begin
  Result := TSQLInvoiceItem.CreateAndFillPrepare(g_InvoiceItemDB.Orm, 'TaskID = ?', [ATask.ID]);
end;

function GetInvoiceTaskFromHullNoNOrderNo(AHullNo, AOrderNo: string): TSQLInvoiceTask;
begin
  Result := TSQLInvoiceTask.CreateAndFillPrepare(g_InvoiceProjectDB.Orm,
    'HullNo = ? AND Order_No = ?', [AHullNo, AOrderNo]);
...
function GetInvoiceItemFromUniqueID(AUniqueID: string): TSQLInvoiceItem;
begin
  Result := TSQLInvoiceItem.CreateAndFillPrepare(g_InvoiceItemDB.Orm,
...
begin
  LItem := TSQLInvoiceItem.CreateAndFillPrepare(g_InvoiceItemDB.Orm,
    'TaskID = ?', [AItem.TaskID]);
...
begin
  ...
  begin
    ...
    begin
      ...
      LStr := LStr + g_GSEngineerType.ToString(AItem.GSEngineerType);
...
function GetInvoiceFileFromID(ATaskID, AItemID: TID): TSQLInvoiceFile;
begin
  Result := TSQLInvoiceFile.CreateAndFillPrepare(g_InvoiceFileDB.Orm, 'TaskID = ? and ItemID = ?', [ATaskID, AItemID]);
end;

function GetSubConInvoiceFileFromSubConIDNItemID(ASubConID, AItemID: TID): TSQLSubConInvoiceFile;
begin
  Result := TSQLSubConInvoiceFile.CreateAndFillPrepare(g_SubConInvoiceDB.Orm, 'SubConID = ? and ItemID = ?', [ASubConID, AItemID]);
end;

function GetSubConInvoiceItemFromUniqueSubConID(AUniqueSubConID: RawUTF8): TSQLSubConInvoiceItem;
begin
  Result := TSQLSubConInvoiceItem.CreateAndFillPrepare(g_SubConInvoiceDB.Orm, 'UniqueSubConID = ?', [AUniqueSubConID]);
...
function GetSubConInvoiceFileFromUniqueSubConID(AUniqueSubConID: RawUTF8): TSQLSubConInvoiceFile;
begin
  Result := TSQLSubConInvoiceFile.CreateAndFillPrepare(g_SubConInvoiceDB.Orm, 'UniqueSubConID = ?', [AUniqueSubConID]);
...
function GetSubConInvoiceItemFromUniqueItemID(AUniqueItemID: RawUTF8): TSQLSubConInvoiceItem;
begin
  Result := TSQLSubConInvoiceItem.CreateAndFillPrepare(g_SubConInvoiceDB.Orm, 'UniqueItemID = ?', [AUniqueItemID]);
...
function GetSubConInvoiceFileFromUniqueItemID(AUniqueItemID: RawUTF8): TSQLSubConInvoiceFile;
begin
  Result := TSQLSubConInvoiceFile.CreateAndFillPrepare(g_SubConInvoiceDB.Orm, 'UniqueItemID = ?', [AUniqueItemID]);
...
function GetSubConInvoiceFileFromUniqueInvoiceFileID(AUniqueInvoiceFileID: RawUTF8): TSQLSubConInvoiceFile;
begin
  Result := TSQLSubConInvoiceFile.CreateAndFillPrepare(g_SubConInvoiceDB.Orm, 'UniqueInvoiceFileID = ?', [AUniqueInvoiceFileID]);
end;

function GetCompanyFromCode4InvoiceTask(ACompanyCode: string): TSQLCompany;
begin
  if ACompanyCode = 'ALL' then
    Result := TSQLCompany.CreateAndFillPrepare(g_InvoiceProjectDB.Orm,
      'ID <> ?', [-1])
  else
    Result := TSQLCompany.CreateAndFillPrepare(g_InvoiceProjectDB.Orm,
...
begin
  LFile := TSQLInvoiceFile.CreateAndFillPrepare(g_InvoiceFileDB.Orm,
