...
uses Winapi.Windows, Winapi.Messages, System.SysUtils, System.Variants,
  ...
  thundax.lib.actions_pjh, UnitMacroRecorderMain2,
  UnitMacroListClass2;
...
var
  LAction: IAction;
begin
  LAction := TAction<String>.Create(atMessage, TParameters<String>.Create(ACompanyCode, ''), '');
  try
    LAction.Execute;
  finally
    LAction := nil;
...
var
  LAction: IAction;
begin
  LAction := TAction<String>.Create(atMessage, TParameters<String>.Create(ARFQ, ''), '');
  try
    LAction.Execute;
  finally
    LAction := nil;
...
begin
  for i := 0 to AKind do
  begin
    LAction := TAction<String>.Create(atKey, TParameters<String>.Create('DOWN', ''), '');
    try
      LAction.Execute;
      Sleep(500);
    finally
  ...
  LAction := TAction<String>.Create(atKey, TParameters<String>.Create('RETURN', ''), '');
  try
    LAction.Execute;
      Sleep(200);
  finally
...
var
  LAction: IAction;
begin
  LAction := TAction<Integer>.Create(atMousePos, TParameters<Integer>.Create(Ax, Ay), '');
  try
    LAction.Execute;
    Sleep(200);
  finally
...
var
  LAction: IAction;
begin
  LAction := TAction<String>.Create(atMouseLClick, TParameters<String>.Create('', ''), '');
  try
    LAction.Execute;
    Sleep(200);
  finally
