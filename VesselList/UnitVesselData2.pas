...
//KF23     KF25      KF21      KH2S      KD21           KF27  (Usage)
  shptSCR, shptBWTS, shptFGSS, shptCOPT, shptPROPELLER, shptEGR, shptVDR,
  shptMarinePackage, shptEGCS, shptHiCONis_ICMS, shptFinal);
...
const
  ...
  R_ShipProductType : array[Low(TShipProductType)..High(TShipProductType)] of string =
         ...
         'Marine Package',
         'EGCS',
         'HiCONiS Integrated Control & Alarm Monitoring System',
         ''
         );
...
function GetProdCodeFromProdType(AProdType: string): string;
...
function GetProdCodeFromProdType(AProdType: string): string;
var
  LProdType: TShipProductType;
begin
  LProdType := g_ShipProductType.ToType(AProdType);
  Result := g_ShipProductCode.ToString(LProdType);
end;
