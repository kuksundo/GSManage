...
uses
  ...
  CommonData2, UnitElecMasterData;
...
function GetElecMasterFromHullNo(const AHullNo: string): TSQLElecMaster;
begin
  Result := TSQLElecMaster.CreateAndFillPrepare(g_ElecMasterDB.Orm,
    'HullNo LIKE ?', ['%'+AHullNo+'%']);
...
begin
  ...
  try
    ...
    Result := TSQLElecMaster.CreateAndFillPrepare(g_ElecMasterDB.Orm, Lwhere, ConstArray);
